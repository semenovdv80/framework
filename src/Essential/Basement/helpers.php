...
{
    if (is_array($data)) extract($data, EXTR_OVERWRITE);//создаем переменные из ключей массива

    $view_path =  str_replace(".", "/", $view_path);//путь к виду, если в пути точки меняем на слеш
    $view_path = $_SERVER['DOCUMENT_ROOT'].'/views/'.$view_path.'.php';
        ...
        if (!empty($includ_match[0]))
        {
            foreach ($includ_match[0] as $includ){
                ...
                if (!empty($includ_patch[0])) {
                    ...
                    include $_SERVER['DOCUMENT_ROOT'].'/views/'.$patch.'.php';
    ...
    if (!empty($layo_match[0]) && !empty($sect_match[0])) {
        ...
        if (!empty($layo_path[0])) {
            ...
            ob_start();
            include_once ('views/' . $layout_path . '.php');//начинаем строить вывод с layout шаблона
